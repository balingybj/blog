...
一般各个 Linux 发行版官方系统源里维护的 Docker 版本都比较低。另外 Docker 官方自己有一个 Docker Englie 仓库，地址是`https://apt.dockerproject.org/`，里面为各大主流的系统平台维护了 Docker Engline 安装包，而且版本都是与时俱进的。各平台的安装文档参见 Docker 官方文档——[Install Docker Engline](https://docs.docker.com/engine/installation/)。其中 Raspbian 的安装过程参见[Install on Raspbian](https://docs.docker.com/engine/installation/linux/raspbian/)。官方还提供了一个安装脚本，可以自动化在各个 Linux 发行版上的安装 Docker 过程。运行`$ curl -sSL https://get.docker.com/ | sudo sh` 既可。
